...
public:
	...
	bool SCreate()
	{
		...
		if (hS == NULL) {
			std::cout << "(!) Failed to create scatter handle 1" << std::endl;
			return false;
		}
		...
		if (hS2 == NULL) {
			std::cout << "(!) Failed to create scatter handle 2" << std::endl;
			return false;
		}
		...
		if (hS3 == NULL) {
			std::cout << "(!) Failed to create scatter handle 3" << std::endl;
			return false;
		}
		...
		if (hS4 == NULL) {
			std::cout << "(!) Failed to create scatter handle 4" << std::endl;
			return false;
		}
		...
		if (hS5 == NULL) {
			std::cout << "(!) Failed to create scatter handle 5" << std::endl;
			return false;
		}
	...
	ULONG64 PatternScan(const char* signature, uint64_t range_start, uint64_t range_end, int PID);
...
bool InitKeyboard() // updated and used
{
	...
	if (Winver > 22000)
	{
		...
		for (size_t i = 0; i < pids.size(); i++)
		{
			...
			if (!VMMDLL_Map_GetModuleFromNameW(DMA_HANDLE, pid, const_cast<LPWSTR>(L"win32ksgd.sys"), &win32k_module_info, VMMDLL_MODULE_FLAG_NORMAL))
			{
				if (!VMMDLL_Map_GetModuleFromNameW(DMA_HANDLE, pid, const_cast<LPWSTR>(L"win32k.sys"), &win32k_module_info, VMMDLL_MODULE_FLAG_NORMAL))
				{
			...
			//win32ksgd
			auto g_session_ptr = PatternScan("48 8B 05 ? ? ? ? 48 8B 04 C8", win32k_base, win32k_base + win32k_size, pid);
			if (!g_session_ptr)
			{
				//win32k
				g_session_ptr = PatternScan("48 8B 05 ? ? ? ? FF C9", win32k_base, win32k_base + win32k_size, pid);
				if (!g_session_ptr)
				{
			...
			int relative = Read<int>(g_session_ptr + 3, pid);
			...
			if (!VMMDLL_Map_GetModuleFromNameW(DMA_HANDLE, pid, const_cast<LPWSTR>(L"win32kbase.sys"), &win32kbase_module_info, VMMDLL_MODULE_FLAG_NORMAL))
			{
			...
			uintptr_t ptr = PatternScan("48 8D 90 ? ? ? ? E8 ? ? ? ? 0F 57 C0", win32kbase_base, win32kbase_base + win32kbase_size, pid);
			uint32_t session_offset = 0x0;
			if (ptr)
			{
				session_offset = Read<uint32_t>(ptr + 3, pid);
...
void UpdateKeys() // updated and used
	{
	...
	bool IsKeyDown(uint32_t virtual_key_code) // updated and used
	{
		if (gafAsyncKeyStateExport < 0x7FFFFFFFFFFF)
			return false;
