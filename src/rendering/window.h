...
bool InitWindow(HINSTANCE instance, INT cmd_show)
{
	...
	else if (settings::config::Topmost) {
	...
	*/
	...
	// Windows
	colors[ImGuiCol_WindowBg] = ImVec4{ 0.07, 0.07f, 0.09f, 1.00f };
	...
	fonts::icons_font = io.Fonts->AddFontFromMemoryTTF(fonts::icons, sizeof(fonts::icons), 1.712962962962963 * (settings::window::Height / 100));

	return true;
}

bool UpdateWindow(void (*mainfunc)()) {
	static bool lastTopmost = settings::config::Topmost; // Track last state

	// Check if topmost state has changed
	if (lastTopmost != settings::config::Topmost) {
		LONG_PTR exStyle = GetWindowLongPtr(window, GWL_EXSTYLE);
		if (settings::config::Topmost) {
			exStyle |= WS_EX_TOPMOST;
		...
		lastTopmost = settings::config::Topmost; // Update last state
	}
